...
function rangeFromTo(f, t) {
  return [...Array(t - f).keys()].map((i) => i + f);
}
...
export default Vue.extend({
  ...
  methods: {
    ...
    save() {
      ...
      if (splited.length > 1) {
        const newElements = (rangeFromTo(parseInt(splited[0].trim(), 10), parseInt(splited[1].trim(), 10)) || []).map((id) => ({ id, _t: Date.now() }));
        this.data.tickets.push(...newElements);
      } else {
        this.data.tickets.push({ id: parseInt(this.inputTicketNumbers, 10), _t: Date.now() });
      }
